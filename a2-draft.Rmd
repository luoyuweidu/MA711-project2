...
In the following session, we will first analyze the 2-cluster analysis in details. 
```{r}
...
```
From the output for the two cluster solutions, the first cluster has 4524 observations while the second cluster has only 4 observations. With the only 4 observations in the second cluster, we can get very limited information from 2-cluster solution. So we decided to use the hierachical method with 3 cluster solutions as our optimal cluster choice. 
