...
To make the dataset applicable for association rules mining, we needed to encode all the numeric variables into categorical variables with appropriate levels. We used the following function *make.ntiles* to perform this conversion.
```{r}
...
```
  With the above function, we were able to convert each numeric variable into a factor variable with n levels. In this stage, we decided to encode all the numeric variables into 3 categories, thus n =3. 
...
```
  ased on the output, the first cluster has 2992 observations, the second cluster has 1532 observations while the third cluster has 4 observations. 
