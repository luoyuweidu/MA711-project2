...
Considering the column X, UNITID, INSTNM_factor and MD_EARN_WNE_P10 are not related to our analysis in this stage, we removed these three variables from the dataset. Then we renamed the column names to make them more readable. The output returned is clean data set we can use in the later analysis.


"we removed these three columns and then removed all the records with any missing values."??????????"
...
To make the dataset applicable for association rules mining, we need to encoded all the numeric variables into categorical variables with appropriate levels. We use the function *make.ntiles* to implement the conversion.  
...
With the above functions, we are able to convert all the numeric variables into a factor variable with n levels. 
...
data_asso %>% 
  sapply(.,is.numeric) %>%
    data_asso[,.] %>%
      apply(.,make.ntiles, n=4, MARGIN = 2) %>%
         as.data.frame()   %>%
             {.} -> data_num
data_combine = cbind(data_num,data_asso[,c(1,2,3)])
...
## 2.3 Objective 
   ...
   Our two objectives for this part of analysis are: 
      1. To identify factors that have notable relationship with post-earning so that they can be used later to predict and influence earning.
      2. To Describe the profile of university awarding different types degree with related attributes. 
    
## 2.4 Association Rule Analysis - Median earning six years after entry

We used the following code to produce the association rules for the data_clean dataset. We used the parameter, appearance and control parameters of the apriori command below are use to modify the rules it generates. The appearance parameter indicates that the consequent (right hand side) of all generated rules should be one of the three values of the median_earning_6years variable. The parameter parameter indicates that all rules generated by this command must have support greater than 0.01 and confidence greater than 0.01. The minlen and maxlen parameters specify that all generated rules contain 2 ~5 values. ###Since the consequent (right hand side) always has one value then the antecedent (left hand side) will always have one (1) or two (2) values.###
...
apriori.parameter_e = list(support=0.1,
                         confidence=0.8,minlen=2, maxlen=3)
...
```
There are 20 rules generated based on the parameters we specified before. Sometimes, wen a rule (such as #2) is a super rule of another rule (#1) and the former has the same or a lower lift, the former rule (#2) is considered to be redundant. Considering that the redundant rules provides no extra information in addition to the other rule, we used the following code to determine and redundant rules in our resulting rules. 

```{r}
# subset.matrix = is.subset(rules_earning, rules_earning)
# subset.matrix[lower.tri(subset.matrix, diag = T)] = NA
# redundant = colSums(subset.matrix,na.rm = T) >=1
# which (redundant)
```
Based on the output, there are no redundant rules in this case.Thus all the 20 rules have provided valid information. 
...
rules_support = sort(rules_earning,by="support", decreasing = T)
inspect(rules_support[1:5])
```
Based on the output, the high values of support 

```{r}
rules_confidence = sort(rules_earning,by="confidence", decreasing = T)
inspect(rules_support[1:5])
```
