...
```
From the output, all the 5 resulting rules have the lift value large than 1, indicating that all five rules are useful in finding the consequent item sets. The acceptable value for support and confidence are all indication of notable relationship between values of variables. 
...
#3. Cluster Analysis 
## 3.1 Goal
   In general, the goal of cluster analysis is to find groups (clusters) of rows where
...
In order for a yet-to-be-chosen algorithm to group observations together, we first defined some notion of (dis)similarity between observations. Considering that most popular choices for clustering like Euclidean distance is only valid for continuous variables, we need to convert the factor variables into binary dummy variables. 
...
All the columns in the dataset data_clustering are continuous variables, which are applicable for calculating the (dis)similarity between observations. To ensure the clustering results are able to reveal the valuable information to maximum, we decided to standardize every numeric variable in the dataset. 
...
glimpse(data_final)
kNNdistplot(data_final, k = 3)
dbscan.result = dbscan(data_final,eps=2.5,MinPts=4)
