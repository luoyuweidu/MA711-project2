...
* 2.[Association Rule Analysis](#AR)
  ...
  * 2.5 [Conclusions -  Association Rules Analysis for Median earning six years after entry](#AR_conclusion1)
  * 2.6 [Association Rule Analysis - Predominant Degree](#AR_pd)
  * 2.7 [Conclusions -  Association Rules Analysis for Predominant Degree](#AR_conclusion2)
* 3.[Cluster Analysis](#CA)
  * 3.1 [Goal](#CA_goal)
  * 3.2 [Data Preparation](#CA_dp)
  * 3.3 [Objective](#CA_objective)
  * 3.4 [Cluster Analysis -K means](#CA_K)
  * 3.5 [Cluster Analysis -PAM](#CA_pam)
  * 3.6 [Cluster Analysis -Hierarchical](#CA_hc)
  * 3.7 [Cluster Validation and Comparison](#CA_conclusion)
  * 3.8 [The Interpretation for 3 cluster hierachical solutions](#CA_Intepretation)
  * 3.9 [Findings and Conclusions](#CA_findings)
* 4 [Further Studies](#furtherStudies)
...
## 2.5 Conclusions -  Association Rules Analysis for Median earning six years after entry 
...
## 2.6 Association Rule Analysis - Predominant Degree
...
## 2.7 Conclusions - Association Rules Analysis for Predominant Degree
...
```
The matrix *ummy_preddeg* and *dummy_control* contain the 5 binay dummy variables converted from the factor variables PREDDEG_factor and Control_facto. 
...
## 3.4 Cluster Analysis process - K-menas
...
Then we use k means function with the cluster number of 2, repeating 20 times. 
```{R}
km_result <- kmeans(data_final, 3, nstart = 20)
km_result2 = kmeans(data_final, 2, nstart = 20)

data_final%>%
  mutate(cluster = factor(km_result$cluster), cluster2=factor(km_result2$cluster)) %>%
   {.} -> data_km
...
subset(data_km,cluster2==2)
subset(data_km,cluster2==1)

subset(data_km,cluster==2)
subset(data_km,cluster==1)
subset(data_km,cluster==3)
...
We produced the cluster memeberships for both two and three clusters as follows: 
```{r}
data_final %>%
  mutate(hc_cluster2 = factor(cutree(hclust_result,k=2)),hc_cluster3 =factor(cutree(hclust_result,k=3)) ) %>%
   {.} -> data_hc
```
The solution was recorded respectively in a new column hc_cluster2 and hc_cluster3, the updated data set was named with data_hc. Before we dig deeper into the interpretations for cluster solutions, we will use *clValid* function in the folliwng section to select the optimum cluster methods. 


## 3.7 Cluster Validations and Comparisons 
...
As we stated in section XXXX, the 2 cluster hierarchical solutions and 3 cluster hierarchical sulutions are stored in the data set data_hc. In the following session, we will first analyze the 2-cluster analysis in details. 
```{r}
...
```
From the output for the two cluster solutions, the first cluster has 4524 observations while the second cluster has only 4 observations. It makes no sense considering the serious uneven distrubution among the two clusters. With the only 4 observations in the second cluster, we can get very limited information from 2-cluster solution. So we decided to use the hierachical method with 3 cluster solutions as our optimal cluster choice. 

## 3.8 The Interpretation for 3 cluster hierachical solutions 
We used the following codes to generate the histogram for Hierachical 3 Cluster Solutions.
```{r}
...
Considering the third cluster only has 4 observations, we would like to directly intepretat the this cluster by looking at the labels of this cluster. 
```{r}
#select the cluster 3 membership from the original dataset
subset(data_c, UNITID %in% rownames(subset(data_hc,hc_cluster3==3)))[,-1]
```
Based on the output, we can tell that all of these four universities are all Frivate For-proft with the prededominant degree not classified. There is no other attributes we found common to characterize the clusters. 


Then we calculated each variable's median value aggregated by the three clusters.
```{r}
...
subset(data_c, UNITID %in% rownames(subset(data_hc,hc_cluster3==3)))[,-1]
```
From the output, we can tell that the cluster 1 are the TITLE IV institutions with low average net price, high percentage of students who are financially independent, high percentage first-generation students, low median family income, high percentage of Pell Grant receiving students, high percent of all federal undergraduate students receiving a federal student, high median debt for students who have completed, low median debt for sutdents who have completed, low median debt for sutdents who have not completed, low median income six years after entry, low median income ten years after entry, low repayment rate 3 years after entering repayment, high loan default rate 3 years afer entering repayment. There is no obvious shared characteristic in terms of the institution's control type or predominant degree in cluster 1. For the institutions in cluster 2, the direction for most columns' median value are opposite to those in the cluster 1. 


To analyze the variables of interest under the Hierachical 3 cluster solutions, we used the ggplot functions to plot the visualizations for certain variable(s).  Below is the code to generate the scatter plot between PCTPELL and MD_EARN_WNE_P under the hierachical 3 cluster solutions.
```{R}
#ggplot
data_hc %>% 
  ggplot(aes(x=MD_EARN_WNE_P6, y=PCTPELL)) + 
...
```
From the scatter plot, it is easy to see that the instituions in cluster 1 have a high percentage of students who receive the Pell Grant meanwhile their median earnings 6 years after the enrollment is relative low compared to the records in the cluster 2. 
...
Based on the 3 cluster hierarchical solutions, we are able to conclude that 
#4 Further Studies
