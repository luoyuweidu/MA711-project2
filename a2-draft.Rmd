...
```
  ...
  Then we applied the make.ntiles function on every numeric column. The resulting output would be a set of variables encoded from numeric variables according to their quantiles. The Q1 is representative of the range for low value, the Q2 is representative of the range for medium value and the Q3 is representative of the range for high value. Then we combined the encoded variables with the original three categorical variables in the dataset to generate a complete dataset. 
```{r}
  data_asso %>% 
    sapply(.,is.numeric) %>%
      data_asso[,.] %>%
...
Then we used some visualizations to better understand these 195 rules. We drawed the scatter plot to visualize the relationship among support, confidence and lift. 
```{r}
...
```
  The matrix *ummy_preddeg* and *dummy_control* contain the 5 binary dummy variables converted from the factor variables PREDDEG_factor and Control_facto. 
...
```
Based on the output, we can tell that all of these four universities are all Private For-profit with the predominant degree not classified. There is no other attributes we found common to characterize the clusters. One assumption might be that these four observations are the outliers within the data set; We will include this assumption's validation in our future studies. 
