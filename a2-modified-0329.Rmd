...
At the first stage(Assignment1), we've explored the relationships between pairs of variables in the College Scorecard data set. In this analysis, we mainly focused on two variables we were most interested in: the students' **post-graduation earnings** and the institutions' **predominant degree types**. We used Association Rule Analysis to explore the relationships between post-graduation earnings, predominant degree and the other variables. The clean data set in Assignment 1 -  ```data_clean.csv``` was the data set we used in this study. 
...
```
  ...
  With the above function, we converted each numeric variable into a factor variable with n levels. In this case, we decided to encode all the numeric variables into 3 categories, thus n = 3. 
...
## 2.3 Objective<a id="AR_objective"></a> 
  ...
  Specifically, we have two objectives in this part:          
    1.To identify the attributes that have notable relationships with the post-graduation earnings so that these variables can be used to predict the post-graduate earnings in the future.         
...
## 2.4 Association Rule Analysis - Median earnings six years after entry<a id="AR_md6"></a>
  ...
  The ```appearance``` parameter set the right-hand side(RHS) and left-hand side(LHS) of rules. In this case, we obtained the rules with RHS limited to the values in column ```median_earning_6years```.
  ...
  The ```control``` parameter decided whether the progress of generating rules would be displayed.
...
```
  ...
  In total, there are **95 rules**, given the parameters we specified before. 
...
The confidence value is 0.8091, indicating that 80.91% of the universities whose students have high median family income, high debt amount before graduation and high repayment rate tend to have students with high median earning 6 years after the enrollment. 
...
Lastly, we sorted the rules by lift value and further inspect the top 5 rules. 
...
Top 5 rules are all about Bachelor degree. A high confidence indicates a strong relationship between LHS and RHS, so we can say that if we observe the existence of LHS, the RHS is very likely to happen. For example, for the first rule, if a institution has low percentage of first-generation students, low percentage of students receiving the pell grant, high median family income, good repayment condition and more students applying to more than 2 school, this institution is 97% likely to provide Bachelor degree as predominant degree.

We then sorted all 195 rules by the lift value. If the lift value equals to 1, it would imply that the probability of the occurrence of LHS and the probability of occurrence of the RHS are independent. With that said, the higher the lift value is, the more strongly the LHS and RHS are associated.
...
## 3.2 Data Preparation<a id="3.2"></a>
  ...
  ```University name```, ```ID```, and ```state columns``` are not useful in Clustering Analysis since they are either identifiers or variable with too many levels, thus we removed these three columns along with the index column generated by R when importing data.
...
```
   ...
   The ```dummy_preddeg``` and ```dummy_control``` are dummy variables for ```PREDDEG_factor``` and ```Control_factor```.
  
   In order to make the column name of 8 dummy variables easy to understand, we renamed these variables with the appropriate names and then combined them with the original 13 continuous variables. We also removed the records with any missing value since the clustering algorithms could not handle missing values. 
...
```
  ...
  In the data set ```data_clean_c```, all variables are now numeric and appropriate for Clustering Analysis. 
   ...
   To ensure that every variable receives the same weight, we then standardized the numeric variables in the data set. We also noticed the ```Graduate-degree``` only contained 0, which means that it couldn't provide any additional information. So, we removed this column.
...
```
   ...
   The resulting ```data_final``` is the clean data set for analysis, with 4,528 observations and 20 columns. 
...
## 3.4 Cluster Analysis process - K-means<a id="3.4"></a>
  ...
  We set the seed value to 100 so that we could reproduce the analysis later.
...
```
    ...
    From the scree plot, we can tell that the 2-cluster or 3-cluster solution is worth trying. We chose k=3 for the  detailed analysis here but we would also consider the 2-cluster solution in the evaluation stage using ```clValid```. 
...
```
  ...
  The corresponding cluster solution was saved in a new column named ```cluster```. The new data set was named as ```data_km```.
  
  We also used the ```dplyr``` function to examine the size of each cluster.
...
## 3.5 Cluster Analysis process - Partitioning Around Mediods (PAM)<a id="3.5"></a>
  ...
  To determine the optimal number of clusters for Partitioning Around Mediods, we calculated the *silhouette* width for cluster numbers from 2 to 10 and plotted a scree plot. 
...
A dengogram was also generated to help select the number of clusters. Due to the large size of our data set, there was not much useful information we could extract from the bottom part of the dengogram. However, there are clearly two very distinct groups at the top of the dengogram. It looks like either two or three groups might be a good place to start investigating. 
...
```
  ...
  The clustering result for each observation was stored in two new columns named ```hc_cluster2``` and ```hc_cluster3```, the updated data set was named with ```data_hc```. Before we interpreted the cluster solutions, we used ```clValid``` function in the following section to validate and finalize the optimum cluster solution.

  
## 3.7 Cluster Validations and Comparisons<a id="3.7"></a> 
  
To compare the different clustering methods and choose the optimum number of clusters, we ran a cluster validation using ```clValid``` function. This function allows us to compare the multiple clustering evaluation metrics under different clustering methods as well as different cluster numbers.
...
We first looked at the third cluster to see what makes these observations different from others.
...
```
 ...
 Based on the output, we can tell that all of these four universities are all private for-profit with a  *Not-Classified* predominant degree. Institutions in this cluster have a higher percent of pell grant, higher loan amount and a higher percent of first-generation students compared to the other two clusters. 
...
```
   ...
   From the scatter plot, it is easy to tell that the institutions in the cluster 1 have more students who received the Pell Grant meanwhile the students' median earnings 6 years after the enrollment are relative low compared to the institutions from the cluster 2. 
...
```
   ...
   It's evident that the institutions in cluster 2 have higher earnings than those in cluster 1 and 3. Moreover, about 70% of values in cluster 1 are above the average earning for all the observations. The earnings values for cluster 1 and 3 roughly fall in a similar range while the observations in cluster 1 spread more widely. In addition, most of the observations in cluster 1 and 3 have a earning value below the average.
...
## 3.9 Findings and Conclusion<a id="3.9"></a> 
